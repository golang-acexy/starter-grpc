...
func checkConn(target string, c *Conn, waitConnReady bool, opts ...grpc.DialOption) (*Conn, error) {
	if waitConnReady {
		logger.Logrus().Traceln("waitConnToReady = true, check conn state....")
		if !c.IsConnReady() {
			i := 0
			for !c.IsConnReady() {
				if i <= 10 {
					logger.Logrus().Warningln("conn state not ready, still wait ...")
					if c.IsConnReady() {
						break
					}
				} else {
					_ = c.gCon.Close()
					logger.Logrus().Warningln("conn state not ready, released old conn get new conn")
	...
	logger.Logrus().Traceln("waitConnToReady = true, conn ready to connect")
	return c, nil
}
